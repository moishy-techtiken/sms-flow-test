...
exports.appointmentBookingActions = void 0;
const PAGE_SIZE = 9;
function getNumberValue(context, key, fallback = 0) {
    const value = context[key];
    return typeof value === "number" && Number.isFinite(value) ? value : fallback;
}
function getStringValue(context, key) {
    const value = context[key];
...
function getLocationChoice(context) {
...
function getTimeChoice(context) {
...
function buildPagedOptions(input) {
    ...
    const lines = pageItems.map((item, index) => `${index + 1}. ${input.getLabel(item)}`);
...
exports.appointmentBookingActions = {
        ...
        try {
            ...
            return {
                ...
                contextPatch: {
                    ...
                    appointmentLocationPromptText: "Let's book your appointment. Reply with a location number.",
        ...
        if (selectedChoice.kind === "more") {
            ...
            return {
                ...
                contextPatch: {
                    ...
                    appointmentLocationOptions: page.options,
                    appointmentLocationOptionsText: page.text,
                    appointmentLocationPromptText: "Reply with a location number.",
                    appointmentLocationChoice: null
                }
            };
        }
    ...
    async loadAppointmentDays({ context, services }) {
        ...
        const selectedLocationName = getStringValue(context, "selectedLocationName") ?? "the selected location";
    ...
    async handleDaySelection({ context }) {
        ...
        if (selectedChoice.kind === "more") {
            ...
            const selectedLocationName = getStringValue(context, "selectedLocationName") ?? "this location";
            const page = buildPagedOptions({
                items: schedules,
                page: nextPageNumber,
                nextStep: "handleDaySelection",
        ...
        try {
            ...
            if (status === 403) {
                return {
                    ...
                    next: "welcome",
                    contextPatch: {
                        appointmentLocationPromptText: "That location is closed for bookings. Reply with another location number.",
                        selectedLocationId: null,
                        selectedLocationName: null,
                        selectedScheduleId: null,
                        selectedDate: null,
